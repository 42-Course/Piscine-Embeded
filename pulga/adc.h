...
static inline void adc_set_reference(uint8_t ref_bits) {
  const uint8_t mask = _BIT(REFS0) | _BIT(REFS1);

  if (ref_bits & ~mask) {
#ifdef PULGA_CHECKS
    utils_warn("adc_set_reference(): invalid ref_bits value");
#endif
    ref_bits &= mask;
  }

  ADMUX = (ADMUX & ~mask) | (ref_bits & mask);
  __adc_state.ref = ref_bits & mask;
}
...
static inline void adc_set_prescaler(uint8_t presc_bits) {
  const uint8_t mask = _BIT(ADPS0) | _BIT(ADPS1) | _BIT(ADPS2);

  if (presc_bits & ~mask) {
#ifdef PULGA_CHECKS
    utils_warn("adc_set_prescaler(): invalid prescaler bits");
#endif
    presc_bits &= mask;
  }

  ADCSRA = (ADCSRA & ~mask) | (presc_bits & mask);
  __adc_state.presc = presc_bits & mask;
}
...
static inline void adc_select_channel(uint8_t ch) {
  if (ch > 7) {
#ifdef PULGA_CHECKS
    utils_warn("adc_select_channel(): invalid channel (0–7)");
#endif
    ch &= 0x07;
  }

  const uint8_t mask = 0x1F;  /* keep REFS + ADLAR intact */
  ADMUX = (ADMUX & ~0x0F) | (ch & 0x0F);
}
